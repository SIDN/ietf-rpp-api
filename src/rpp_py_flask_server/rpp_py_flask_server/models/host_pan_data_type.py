from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from rpp_py_flask_server.models.base_model import Model
from rpp_py_flask_server.models.epp_tr_id_type import EppTrIDType
from rpp_py_flask_server.models.host_pa_name_type import HostPaNameType
from rpp_py_flask_server import util

from rpp_py_flask_server.models.epp_tr_id_type import EppTrIDType  # noqa: E501
from rpp_py_flask_server.models.host_pa_name_type import HostPaNameType  # noqa: E501

class HostPanDataType(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, name=None, pa_date=None, pa_trid=None):  # noqa: E501
        """HostPanDataType - a model defined in OpenAPI

        :param name: The name of this HostPanDataType.  # noqa: E501
        :type name: HostPaNameType
        :param pa_date: The pa_date of this HostPanDataType.  # noqa: E501
        :type pa_date: datetime
        :param pa_trid: The pa_trid of this HostPanDataType.  # noqa: E501
        :type pa_trid: EppTrIDType
        """
        self.openapi_types = {
            'name': HostPaNameType,
            'pa_date': datetime,
            'pa_trid': EppTrIDType
        }

        self.attribute_map = {
            'name': 'name',
            'pa_date': 'paDate',
            'pa_trid': 'paTRID'
        }

        self._name = name
        self._pa_date = pa_date
        self._pa_trid = pa_trid

    @classmethod
    def from_dict(cls, dikt) -> 'HostPanDataType':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The host_panDataType of this HostPanDataType.  # noqa: E501
        :rtype: HostPanDataType
        """
        return util.deserialize_model(dikt, cls)

    @property
    def name(self) -> HostPaNameType:
        """Gets the name of this HostPanDataType.


        :return: The name of this HostPanDataType.
        :rtype: HostPaNameType
        """
        return self._name

    @name.setter
    def name(self, name: HostPaNameType):
        """Sets the name of this HostPanDataType.


        :param name: The name of this HostPanDataType.
        :type name: HostPaNameType
        """
        if name is None:
            raise ValueError("Invalid value for `name`, must not be `None`")  # noqa: E501

        self._name = name

    @property
    def pa_date(self) -> datetime:
        """Gets the pa_date of this HostPanDataType.


        :return: The pa_date of this HostPanDataType.
        :rtype: datetime
        """
        return self._pa_date

    @pa_date.setter
    def pa_date(self, pa_date: datetime):
        """Sets the pa_date of this HostPanDataType.


        :param pa_date: The pa_date of this HostPanDataType.
        :type pa_date: datetime
        """
        if pa_date is None:
            raise ValueError("Invalid value for `pa_date`, must not be `None`")  # noqa: E501

        self._pa_date = pa_date

    @property
    def pa_trid(self) -> EppTrIDType:
        """Gets the pa_trid of this HostPanDataType.


        :return: The pa_trid of this HostPanDataType.
        :rtype: EppTrIDType
        """
        return self._pa_trid

    @pa_trid.setter
    def pa_trid(self, pa_trid: EppTrIDType):
        """Sets the pa_trid of this HostPanDataType.


        :param pa_trid: The pa_trid of this HostPanDataType.
        :type pa_trid: EppTrIDType
        """
        if pa_trid is None:
            raise ValueError("Invalid value for `pa_trid`, must not be `None`")  # noqa: E501

        self._pa_trid = pa_trid
